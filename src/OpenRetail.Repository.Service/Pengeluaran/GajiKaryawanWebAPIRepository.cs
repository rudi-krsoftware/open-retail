    ...
    public class GajiKaryawanWebAPIRepository : IGajiKaryawanRepository
    {
        ...
        public GajiKaryawan GetByID(string id)
        {
			...
			try
            {
                ...
                var response = client.Execute<OpenRetailWebApiGetResponse<GajiKaryawan>>(request).Data;
        ...
        public IList<GajiKaryawan> GetByBulanAndTahun(int bulan, int tahun)
        {
            ...
            {
                var client = new RestClient(_apiUrl);
                var request = new RestRequest(string.Format("get_by_bulan_and_tahun?bulan={0}&tahun={1}", bulan, tahun), Method.GET);
                var response = client.Execute<OpenRetailWebApiGetResponse<GajiKaryawan>>(request).Data;
